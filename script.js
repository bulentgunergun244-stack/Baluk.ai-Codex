...
function renderEngineWorkspace() {
  ...
  if (enginePrompt) enginePrompt.placeholder = "Bir sey yaz... Orn: merhaba yazisi ekle, buton ekle, yaziyi buyut";
}
...
function buildHtmlActionPlanV2(prompt = "") {
  ...
  const buttonLabel = extractEngineButtonLabels(prompt)[0] || (/(bana tıkla|bana tikla)/.test(lower) ? "Bana tikla" : "Buton");
  const textColor = /mavi|blue/.test(lower) ? "#2563eb" : /yesil|yeşil|green/.test(lower) ? "#16a34a" : /kirmizi|kırmızı|red/.test(lower) ? "#dc2626" : "";
  const wantsClickAction = /(basinca|basınca|tiklayinca|tıklayınca)/.test(lower);
  return {
    addText: (/(yazısı ekle|yazisi ekle|yazısını ekle|yazisini ekle|ekrana .* yaz|baslik|başlık)/.test(lower) || (!!textValue && !/(buton|button)/.test(lower))) && !wantsClickAction,
    addButton: /(buton|button)/.test(lower),
    addInput: /(input|giris|giriş|form)/.test(lower),
    centerText: /(ortala|ortalanmis|ortalanmış|center)/.test(lower),
    growText: /(buyut|büyüt|32px|48px|64px|buyuk|büyük)/.test(lower),
    ...
    roundButton: /(köşelerini yuvarlak yap|koselerini yuvarlak yap|yuvarlak yap|rounded)/.test(lower),
    growButton: /(butonu buyut|butonu büyüt|butonu buyuk yap|butonu büyük yap)/.test(lower),
    recolorButton: /(buton.*mavi|buton.*kirmizi|buton.*kırmızı|buton.*yesil|buton.*yeşil|buton.*rengini değiştir|buton.*rengini degistir)/.test(lower),
    renameButton: /(buton.*yazisini|buton.*yazısını|üstündeki|ustundeki)/.test(lower) && !!buttonLabel,
    textValue,
    buttonLabel,
    textColor
  };
...
function getEngineStatusSteps(prompt = "", model = "baluk-2.2") {
  const lower = String(prompt || "").toLocaleLowerCase("tr-TR");
  const steps = [];
  if (/(merhaba|utku|yazi|yazı|baslik|başlık)/.test(lower)) steps.push({ thinking: "Yaziyi ekliyorum...", done: "Tamam, yaziyi ekledim." });
  if (/(buyut|büyüt|buyuk|büyük|32px|48px|64px)/.test(lower)) steps.push({ thinking: "Yaziyi buyutuyorum...", done: "Tamam, yaziyi buyuttum." });
  if (/(ortala|ortalanmis|ortalanmış|center)/.test(lower)) steps.push({ thinking: "Yaziyi ortaliyorum...", done: "Tamam, yaziyi ortaladim." });
  if (/(buton|button)/.test(lower)) steps.push({ thinking: "Butonu ekliyorum...", done: "Tamam, butonu ekledim." });
  if (/(basinca|basınca|tiklayinca|tıklayınca)/.test(lower)) steps.push({ thinking: "Tiklama davranisini ekliyorum...", done: "Tamam, tiklama davranisini ekledim." });
  if (/(input|giris|giriş)/.test(lower)) steps.push({ thinking: "Inputu ekliyorum...", done: "Tamam, inputu ekledim." });
