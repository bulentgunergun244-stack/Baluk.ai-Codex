...
function renderBlmPreview(source = "") {
  ...
  const slugify = (value = "") => String(value).toLocaleLowerCase("tr-TR").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || `item-${Math.random().toString(36).slice(2, 7)}`;
  ...
  lines.forEach((rawLine) => {
    ...
    if (/^print\s*\(/i.test(line)) {
      ...
      const nameSlug = nameMatch ? slugify(nameMatch[1]) : "";
      const hiddenAttr = nameMatch ? ` data-blm-text="${nameSlug}" style="${escapeHtml(`${style}${style ? ";" : ""}display:none;`)}"` : ` style="${escapeHtml(style)}"`;
      pieces.push(`<p${hiddenAttr}>${escapeHtml(text || "Yazi")}</p>`);
      return;
    }
    if (/^button\s*\(/i.test(line)) {
      ...
      const targetSlug = nameMatch ? slugify(nameMatch[1]) : "";
      pieces.push(`<button class="blm-inline-btn" type="button" data-blm-button="${targetSlug}" onclick="document.querySelectorAll('[data-blm-text]').forEach(function(node){node.style.display='block';});" style="${escapeHtml(style)}">${escapeHtml(text || "Buton")}</button>`);
      return;
    }
...
function renderEngineDock() {
  ...
  if (enginePanelMode === "notebook") {
    ...
    const notebookText = active?.code || active?.notebook || "// Kod geldikce burada duzenleyebilirsin.";
    engineDockBody.innerHTML = `
    ...
    document.getElementById("engineRunNotebookBtn")?.addEventListener("click", () => {
      ...
      if (session.language === "blm") session.notebook = nextText;
      if (session.language === "blm") session.outputHtml = renderBlmPreview(nextText);
  ...
  engineDockTitle.textContent = "Cikti";
...
function renderEngineProjectView() {
  ...
  engineProjectView.innerHTML = `<div class="engine-project-empty"><p>${engineRunning ? "Baluk Engine dusunuyor..." : "Calisma alani hazir. Dusunme animasyonu burada gorunur; ciktilar T panelinde, kodlar N panelinde acilir."}</p></div>`;
}
