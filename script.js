...
{
  const freshEngineBtn = balukEngineBtn ? balukEngineBtn.cloneNode(true) : null;
  if (balukEngineBtn && freshEngineBtn) {
    balukEngineBtn.replaceWith(freshEngineBtn);
    freshEngineBtn.addEventListener("click", openBalukEngine);
  }
}
...
if (engineStudyToggle) engineStudyToggle.addEventListener("click", () => toggleEnginePanel("study"));
...
function normalizeEngineSession(session) {
  return {
    ...
    language: ["html", "python", "blm"].includes(session?.language) ? session.language : "html"
  };
}
...
function updateEngineSelectionMeta() {
  ...
  const languageLabel = active?.language === "python" ? "Python" : active?.language === "blm" ? "BLM" : "HTML/CSS/JS";
...
function renderEngineLog() {
  if (!engineLog || !engineIntro) return;
  const active = ensureActiveEngineSession();
  const entries = Array.isArray(active?.entries) ? active.entries.slice(-3) : [];
  engineLog.innerHTML = "";
  engineLog.classList.toggle("hidden", entries.length === 0);
  entries.forEach((entry) => {
    const card = document.createElement("article");
    card.className = `engine-log-entry ${entry.role || "assistant"}${entry.kind ? ` ${entry.kind}` : ""}`;
    const label = entry.role === "user" ? "Komut" : "Baluk Engine";
    const meta = entry.meta ? `<div class="engine-log-meta">${escapeHtml(entry.meta)}</div>` : "";
    card.innerHTML = `<strong>${label}</strong><p>${escapeHtml(entry.text || "")}</p>${meta}`;
    engineLog.appendChild(card);
  });
}
function renderBlmPreview(source = "") {
  ...
  const normalizeStyleBlock = (body = "") => body
    .replace(/text-background/gi, "background")
    .replace(/color\s*=/gi, "color:")
    .replace(/background\s*color\s*=/gi, "background:")
    .replace(/background-color\s*=/gi, "background:")
    .replace(/background\s*=/gi, "background:")
    .replace(/padding\s*=/gi, "padding:")
    .replace(/radius\s*=/gi, "border-radius:")
    .replace(/border-radius\s*=/gi, "border-radius:")
    .replace(/font-size\s*=/gi, "font-size:")
    .replace(/font-weight\s*=/gi, "font-weight:")
    .replace(/;/g, "; ")
    .replace(/\s+/g, " ")
    .trim();
  lines.forEach((rawLine) => {
    ...
    if (/^style\s*=/.test(lower)) {
      const typeMatch = line.match(/^style\s*=\s*([a-zA-Z]+)/i);
      const bodyMatch = line.match(/\{([\s\S]*)\}/);
      if (typeMatch && bodyMatch) namedStyles[typeMatch[1].toLowerCase()] = normalizeStyleBlock(bodyMatch[1]);
      return;
    }
    if (/^print\s*\(/i.test(line)) {
      const nameMatch = line.match(/name\s*=\s*([a-zA-Zçğıöşü]+)/i);
      const text = extractQuoted(line);
      const style = namedStyles.text || "";
      namedElements[text] = { type: "text", style };
      pieces.push(`<p style="${escapeHtml(style)}">${escapeHtml(text || "Yazi")}</p>`);
      return;
    }
    if (/^button\s*\(/i.test(line)) {
      const nameMatch = line.match(/name\s*=\s*([a-zA-Zçğıöşü]+)/i);
      const text = extractQuoted(line);
      const style = namedStyles.button || "";
      if (nameMatch) namedElements[nameMatch[1].toLowerCase()] = { type: "button", text, style };
      pieces.push(`<button class="blm-inline-btn" type="button" style="${escapeHtml(style)}">${escapeHtml(text || "Buton")}</button>`);
      return;
    }
    if (/^bal\s*=/i.test(lower)) {
      return;
    }
    if (lower.startsWith("baslik")) {
      pieces.push(`<h2>${escapeHtml(extractQuoted(line) || line.replace(/^baslik\s*/i, ""))}</h2>`);
      return;
    }
    if (lower.startsWith("yaz")) {
      pieces.push(`<p>${escapeHtml(extractQuoted(line) || line.replace(/^yaz\s*/i, ""))}</p>`);
      return;
    }
    if (lower.startsWith("kart")) {
      const quoted = [...line.matchAll(/"([^"]*)"/g)].map((match) => match[1]);
      pieces.push(`<div class="blm-card"><strong>${escapeHtml(quoted[0] || "Kart")}</strong><p>${escapeHtml(quoted[1] || "Icerik")}</p></div>`);
      return;
    }
    if (lower.startsWith("giris")) {
      pieces.push(`<input type="text" placeholder="${escapeHtml(extractQuoted(line) || "Bir sey yaz")}" />`);
      return;
    }
    if (lower.startsWith("topla")) {
      const parts = line.split(/\s+/);
      const a = Number(parts[1] || 0);
      const b = Number(parts[2] || 0);
      pieces.push(`<div class="blm-card"><strong>Toplama Sonucu</strong><p>${a} + ${b} = ${a + b}</p></div>`);
      return;
    }
    if (lower.startsWith("hesapmakinesi")) {
      pieces.push(`<div class="blm-calculator"><strong>BLM Hesap Makinesi</strong><p>Butonlar ve islem akisi bu sahnede gorunur.</p><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:10px;"><button class="blm-inline-btn">7</button><button class="blm-inline-btn">8</button><button class="blm-inline-btn">9</button><button class="blm-inline-btn">+</button></div></div>`);
      return;
    }
    if (lower.startsWith("yilanoyunu")) {
      pieces.push(`<div class="blm-snake"><strong>BLM Snake Arena</strong><p>Bu panel yilan oyununun sahne yerlesimini temsil eder.</p><div style="margin-top:10px;height:160px;border-radius:16px;border:1px dashed #94a3b8;background:linear-gradient(180deg,#0f172a,#111827);"></div></div>`);
      return;
    }
    pieces.push(`<p>${escapeHtml(line)}</p>`);
  });
  return `<div class="blm-stage">${pieces.join("") || ""}</div>`;
}
function renderEngineDock() {
  ...
  [engineNotebookToggle, engineStudyToggle, engineCodeToggle].forEach((btn) => {
    if (!btn) return;
    const isActive = btn.id === "engineNotebookToggle" ? enginePanelMode === "notebook" : btn.id === "engineStudyToggle" ? enginePanelMode === "study" : enginePanelMode === "code";
    btn.classList.toggle("active", isActive);
    btn.setAttribute("aria-pressed", isActive ? "true" : "false");
  });
  if (!enginePanelMode) return;
  if (enginePanelMode === "study") {
    engineDockTitle.textContent = "Study";
    engineDockBody.innerHTML = `
      <div class="engine-study-card">
        <h4>BLM calisma rehberi</h4>
        <p>BLM daha kolay bir deneme dili. Once yaziyi veya butonu tanimlar, sonra istersek stil uygulariz. Study bunu adim adim ogretir.</p>
        <div class="engine-study-stepper">
          <div class="engine-study-step">
            <strong>1. Yazi yazdir</strong>
            <p><code>print("Merhaba dunya")</code> ekrana yazi basar.</p>
          </div>
          <div class="engine-study-step">
            <strong>2. Buton ekle</strong>
            <p><code>button("Tikla")</code> yeni bir buton olusturur.</p>
          </div>
          <div class="engine-study-step">
            <strong>3. Isim ver</strong>
            <p><code>button(name=buton"Baslat")</code> veya <code>print(name=yazi"Merhaba")</code> seklinde elemani tanitabilirsin.</p>
          </div>
          <div class="engine-study-step">
            <strong>4. Stil uygula</strong>
            <p><code>style=button { background=purple color=white padding=12px 18px radius=16px }</code> ya da <code>style=text { color=red font-size=28px }</code></p>
          </div>
        </div>
        <div class="engine-study-grid">
          <button type="button" data-study-insert='print("Merhaba dunya")'>Print kullan</button>
          <button type="button" data-study-insert='button("Tikla")'>Button kullan</button>
          <button type="button" data-study-insert='button(name=buton"Baslat")\nstyle=button { background=purple color=white padding=12px 18px radius=18px }'>Isimli mor buton</button>
          <button type="button" data-study-insert='print(name=yazi"Merhaba")\nstyle=text { color=green font-size=30px font-weight=800 }'>Yaziya stil ver</button>
          <button type="button" data-study-insert='hesapmakinesi'>Hesap makinesi</button>
          <button type="button" data-study-insert='yilanoyunu'>Yilan oyunu</button>
        </div>
        <div class="engine-study-rule">BLM temel komutlari: <b>print("...")</b>, <b>button("...")</b>, <b>button(name=buton"...")</b>, <b>print(name=yazi"...")</b>, <b>style=button { ... }</b>, <b>style=text { ... }</b>, <b>hesapmakinesi</b>, <b>yilanoyunu</b>.</div>
      </div>
    `;
    engineDockBody.querySelectorAll("[data-study-insert]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const session = ensureActiveEngineSession();
        session.language = "blm";
        session.notebook = btn.getAttribute("data-study-insert") || DEFAULT_BLM_NOTEBOOK;
        session.code = session.notebook;
        session.outputHtml = renderBlmPreview(session.notebook);
        session.updatedAt = Date.now();
        enginePanelMode = "notebook";
        renderEngineWorkspace();
        saveEngineSessions();
      });
    });
  ...
  if (enginePanelMode === "notebook") {
    ...
    const notebookText = active?.language === "blm" ? active.notebook : (active?.code || "// Kod geldikce burada duzenleyebilirsin.");
    engineDockBody.innerHTML = `
      <textarea id="engineNotebookArea" class="engine-notebook-area">${escapeHtml(notebookText)}</textarea>
      <button id="engineRunNotebookBtn" class="engine-dock-run" type="button">${active?.language === "blm" ? "BLM'yi calistir" : "Kod notunu kaydet"}</button>
    `;
    ...
    document.getElementById("engineRunNotebookBtn")?.addEventListener("click", () => {
      ...
      if (session.language === "blm") {
        session.notebook = nextText;
        session.code = nextText;
        session.outputHtml = renderBlmPreview(nextText);
        setEngineStatus("BLM guncellendi", "Not defterindeki degisiklikler cikti paneline yansitildi.");
      } else {
        session.code = nextText;
        setEngineStatus("Kod notu guncellendi", "Bu kodu T panelinden kopyalayabilirsin.");
      }
      session.updatedAt = Date.now();
      saveEngineSessions();
      renderEngineDock();
  ...
  engineDockTitle.textContent = active?.language === "blm" ? "Cikti" : "Kod";
  if (active?.language === "blm") {
    engineDockBody.innerHTML = `
      <div class="engine-output-preview">${active.outputHtml || `<div class="blm-stage"><div class="blm-card">BLM cikti hazir degil.</div></div>`}</div>
      <button id="engineCopyNotebookBtn" class="engine-dock-copy" type="button">BLM kodunu kopyala</button>
    `;
    document.getElementById("engineCopyNotebookBtn")?.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(active?.code || active?.notebook || "");
        showWarningOverlay("BLM kodu kopyalandi.");
      } catch {
    ...
    engineDockBody.innerHTML = `
      <pre class="engine-code-block">${escapeHtml(active?.code || "// Henuz kod uretilmedi.")}</pre>
      <button id="engineCopyCodeBtn" class="engine-dock-copy" type="button">Kodu kopyala</button>
    `;
...
function renderEngineProjectView() {
  const active = ensureActiveEngineSession();
  if (engineProjectLanguageBadge) engineProjectLanguageBadge.textContent = active.language === "html" ? "HTML/CSS/JS" : active.language === "python" ? "Python" : "BLM";
  ...
  if (engineProjectTitle) engineProjectTitle.textContent = active.title || "Canli proje";
  if (engineProjectText) {
    engineProjectText.textContent = active.lastPrompt
      ? `Son komut: ${active.lastPrompt}`
      : "Burada proje ciktisi gorunecek. Ayni sohbet boyunca ayni proje uzerinden devam ederiz.";
  }
  if (!engineProjectView) return;
  if (active.language === "html" && active.code) {
    engineProjectView.innerHTML = `<iframe title="HTML proje onizleme" sandbox="allow-scripts" srcdoc="${escapeHtml(active.code)}"></iframe>`;
    return;
  }
  if (active.language === "blm" && active.outputHtml) {
    engineProjectView.innerHTML = `<div class="engine-output-preview clean">${active.outputHtml}</div>`;
    return;
  }
  if (active.language === "python" && active.code) {
    engineProjectView.innerHTML = `<div class="engine-project-python"><pre>${escapeHtml(active.code)}</pre></div>`;
    return;
  }
  engineProjectView.innerHTML = `<div class="engine-project-empty"><p>Proje alani hazir. Komut verince burada canli sonuc goreceksin.</p></div>`;
}
function renderEngineWorkspace() {
  ...
  if (enginePrompt) enginePrompt.placeholder = ensureActiveEngineSession().language === "blm"
    ? "BLM komutu yaz... orn: hesapmakinesi, yilanoyunu, baslik, yaz"
    : "Bir sey yaz... Orn: hesap makinesi yap, yilan oyunu yap";
}
...
function selectEngineLanguage(language) {
  const active = ensureActiveEngineSession();
  active.language = ["python", "blm"].includes(language) ? language : "html";
  if (active.language === "blm" && !active.notebook) active.notebook = DEFAULT_BLM_NOTEBOOK;
  active.updatedAt = Date.now();
  updateEngineSelectionMeta();
  saveEngineSessions();
  renderEngineWorkspace();
  setEngineStatus("Dil guncellendi", `${active.language === "html" ? "HTML/CSS/JS" : active.language === "python" ? "Python" : "BLM"} secildi.`);
}
...
function buildHtmlEngineArtifact(prompt = "", advanced = false) {
...
<body>
  ...
  if (intent.wantsButton) {
    return {
      ...
      code: advanced ? `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8" /><style>body { min-height: 100vh; display: grid; place-items: center; margin: 0; background: linear-gradient(135deg, #e0f2fe, #ede9fe); font-family: Arial, sans-serif; }button { border: none; border-radius: 999px; padding: 16px 26px; font-size: 1rem; font-weight: 800; background: linear-gradient(135deg, #2563eb, #7c3aed); color: #fff; cursor: pointer; }</style></head><body><button id="cta">Baluk butonu</button><script>document.getElementById("cta").addEventListener("click", () => alert("Buton calisti."));</script></body></html>` : `<!DOCTYPE html><html lang="tr"><body><button onclick="alert('Buton calisti')">Tikla</button></body></html>`
    };
  }
  if (intent.wantsHello) {
    return {
      summary: advanced ? "Merhaba ekrani icin ufak bir stil ekledim." : "En sade merhaba cikisini hazirladim.",
      code: advanced ? `<!DOCTYPE html><html lang="tr"><head><meta charset="UTF-8"><style>body { margin: 0; min-height: 100vh; display: grid; place-items: center; background: linear-gradient(135deg, #eff6ff, #f8fafc); font-family: Arial, sans-serif; }p { font-size: 3rem; font-weight: 800; color: #0f172a; }</style></head><body><p>Merhaba</p></body></html>` : `<p>Merhaba</p>`
    };
  }
  return {
...
function buildPythonEngineArtifact(prompt = "", advanced = false) {
  const intent = detectEngineIntent(prompt);
  if (intent.wantsSnake) {
    return {
...
function extendBlmProject(existingCode = "", prompt = "") {
  ...
  if (/merhaba/.test(lower) && !/print\("Merhaba"\)/.test(existingCode)) {
    extra.push(`print("Merhaba")`);
  }
  if (/buton/.test(lower)) {
    extra.push(`button(name=buton"Tikla")`);
  }
...
function buildEngineArtifact(prompt = "", language = "html", model = "baluk-2.2") {
  const advanced = model === "baluk-2.2-thinking";
  if (language === "python") return buildPythonEngineArtifact(prompt, advanced);
  if (language === "blm") return buildBlmEngineArtifact(prompt, advanced);
  return buildHtmlEngineArtifact(prompt, advanced);
}
function getEngineStatuses(language = "html", model = "baluk-2.2") {
  const languageLabel = language === "python" ? "Python" : language === "blm" ? "BLM" : "HTML/CSS/JS";
  return model === "baluk-2.2-thinking"
    ? ["Istegini anladim.", `${languageLabel} baglaniyor...`, "Yapiyi planliyorum...", "Detayli kodu kuruyorum...", "Stil ve guzellik katmani ekleniyor...", "Son kontroller yapiliyor..."]
    : ["Istegini anladim.", `${languageLabel} ortami kuruluyor...`, "Temel parcilar diziliyor...", "Kodu toparliyorum..."];
}
...
async function runEngineStatuses(statuses = [], totalMs = 12000) {
  ...
  for (const step of statuses) {
    setEngineStatus("Baluk Engine calisiyor", step, true);
    appendEngineEntry("assistant", step, "Baluk Engine dusunuyor", "thinking");
...
async function submitEnginePrompt() {
  ...
  engineRunning = true;
  appendEngineEntry("user", prompt, `${active.language === "html" ? "HTML/CSS/JS" : active.language === "python" ? "Python" : "BLM"} • ${active.model === "baluk-2.2-thinking" ? "Thinking" : "Normal"}`);
  enginePrompt.value = "";
  const statuses = getEngineStatuses(active.language, active.model);
  const totalMs = estimateEngineThinkingMs(prompt, active.model);
  await runEngineStatuses(statuses, totalMs);
  let artifact = buildEngineArtifact(prompt, active.language, active.model);
  if (active.language === "blm" && active.code && /(ekle|degistir|düzenle|duzenle|renk|buton|merhaba|yilan|hesap)/i.test(prompt)) {
    const nextCode = extendBlmProject(active.code, prompt);
    artifact = {
      summary: "BLM projesini ayni sohbet uzerinden guncelledim.",
      code: nextCode,
      outputHtml: renderBlmPreview(nextCode)
    };
  }
  active.lastPrompt = prompt;
  active.code = artifact.code;
  active.outputHtml = artifact.outputHtml || "";
  if (active.language === "blm") active.notebook = artifact.code;
  active.updatedAt = Date.now();
  appendEngineEntry("assistant", artifact.summary, `${active.model === "baluk-2.2-thinking" ? "Thinking" : "Normal"} • ${active.language.toUpperCase()}`);
  setEngineStatus(
