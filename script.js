...
function normalizeEngineSession(session) {
  return {
    ...
    language: ["html", "python"].includes(session?.language) ? session.language : "html"
  };
}
...
function renderEngineDock() {
  ...
  if (enginePanelMode === "notebook") {
    ...
    const notebookText = active?.code || "// Kod geldikce burada duzenleyebilirsin.";
...
function renderEngineProjectView() {
  const active = ensureActiveEngineSession();
  if (engineProjectLanguageBadge) engineProjectLanguageBadge.textContent = active.language === "html" ? "HTML/CSS/JS" : "Python";
  ...
  engineProjectView.innerHTML = engineRunning
    ? `<div class="engine-thinking-scene"><div class="engine-thinking-card"><svg class="engine-fish" viewBox="0 0 520 220" fill="none" xmlns="http://www.w3.org/2000/svg"><use href="#baluk-fish"></use></svg><div class="engine-bubbles"><span>&lt;/&gt;</span><span>&lt;/&gt;</span><span>&lt;/&gt;</span></div><div class="engine-thinking-copy"><strong>${escapeHtml(engineStatusStateTitle || "Baluk Engine dusunuyor")}</strong><p>${escapeHtml(engineStatusStateText || "Kod planlaniyor...")}</p></div></div></div>`
    : `<div class="engine-project-empty"><p>Calisma alani hazir. Dusunme animasyonu burada gorunur; kodlar ve cikti T panelinde, duzenleme N panelinde acilir.</p></div>`;
}
function renderEngineWorkspace() {
  ...
  if (enginePrompt) enginePrompt.placeholder = ensureActiveEngineSession().language === "python"
    ? "Bir sey yaz... Orn: python ile 1+1 hesapla, input al, kosul kur"
    : "Bir sey yaz... Orn: mavi bir buton ekle, tiklaninca sayi artsin";
}
...
function selectEngineLanguage(language) {
  const active = ensureActiveEngineSession();
  active.language = language === "python" ? "python" : "html";
  active.updatedAt = Date.now();
  updateEngineSelectionMeta();
  saveEngineSessions();
  renderEngineWorkspace();
  setEngineStatus("Dil guncellendi", `${active.language === "html" ? "HTML/CSS/JS" : "Python"} secildi.`);
}
...
<body>
  ...
  if (intent.wantsButton || intent.wantsTitle || intent.wantsHello || intent.wantsInput || wantsSite || wantsTwoButtons) {
    ...
    const subtitle = advanced
      ? "Temiz bir HTML, dengeli CSS ve gerekli JS davranislarini birlikte kurdum."
      : "Temel yapiyi kurup komutun icin gerekli parcayi ekledim.";
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    p {
      ...
      max-width: 58ch;
      line-height: 1.6;
      color: ${palette.text === "#ffffff" ? "rgba(255,255,255,.84)" : "#475569"};
    }
    .actions {
      display: flex;
      flex-wrap: wrap;
    ...
    .diamond-btn {
      width: ${advanced ? "112px" : "96px"};
      height: ${advanced ? "112px" : "96px"};
      border: none;
      border-radius: 28px;
    ...
    .panel {
      width: min(${advanced ? "720px" : "640px"}, 100%);
      min-height: 150px;
      border-radius: 28px;
      border: 1px solid rgba(255,255,255,.24);
      background: ${palette.text === "#ffffff" ? "rgba(255,255,255,.12)" : "rgba(255,255,255,.78)"};
      box-shadow: 0 20px 42px rgba(15, 23, 42, 0.14);
      padding: ${advanced ? "24px" : "18px"};
      text-align: left;
...
<body>
  <main class="page">
    <h1>${titleText}</h1>
    <p>${subtitle}</p>
    ${intent.wantsInput ? '<div class="panel"><input type="text" placeholder="Buraya bir sey yaz..." /></div>' : ""}
    <div class="actions">
      <button id="actionOne" class="diamond-btn" type="button"><span>${firstLabel}</span></button>
      ${wantsTwoButtons ? `<button id="actionTwo" class="diamond-btn" type="button"><span>${secondLabel}</span></button>` : ""}
    </div>
    <section class="panel output" id="outputPanel">
      <p class="output-message">Butonlara basildiginda sonuc burada gorunur.</p>
    </section>
  </main>
  <script>
    ...
    ${wantsTwoButtons ? `secondButton.addEventListener("click", () => {
...
function getEngineStatuses(language = "html", model = "baluk-2.2") {
  ...
  return model === "baluk-2.2-thinking"
    ? ["Istegini anladim, basliyorum.", `${languageLabel} ortami baglaniyor...`, "Ana yapiyi kuruyorum...", "Gerekli stilleri ve davranislari ekliyorum...", "Kodu daha temiz ve daha guclu hale getiriyorum...", "Son kontrolleri yapiyorum..."]
    : ["Istegini anladim, basliyorum.", `${languageLabel} ortami kuruluyor...`, "Temel parcilar diziliyor...", "Kodu toparlayip teslim ediyorum..."];
}
...
async function submitEnginePrompt() {
  ...
  engineRunning = true;
  appendEngineEntry("user", prompt, `${active.language === "html" ? "HTML/CSS/JS" : "Python"} • ${active.model === "baluk-2.2-thinking" ? "Thinking" : "Normal"}`);
  enginePrompt.value = "";
  ...
  active.updatedAt = Date.now();
  appendEngineEntry("assistant", artifact.summary, `${active.model === "baluk-2.2-thinking" ? "Thinking" : "Normal"} • ${active.language === "html" ? "HTML/CSS/JS" : "PYTHON"}`);
  setEngineStatus(
