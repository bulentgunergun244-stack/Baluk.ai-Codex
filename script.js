...
function renderEngineProjectView() {
  ...
  const feedHtml = visibleEntries.length
    ? `<div class="engine-conversation-feed">${visibleEntries.map((entry) => `
        <article class="engine-conversation-bubble ${entry.role === "user" ? "user" : "assistant"} ${entry.kind === "thinking" ? "thinking" : ""}">
          <strong>${entry.role === "user" ? "Sen" : "Baluk Engine"}</strong>
          <p>${escapeHtml(entry.text || "")}</p>
          ${entry.meta ? `<span>${escapeHtml(entry.meta)}</span>` : ""}
        </article>
      `).join("")}</div>`
    : `<div class="engine-project-empty"><p>Calisma alani hazir. Burada once sohbet akisi gorunur, dusunurken animasyon acilir.</p></div>`;
