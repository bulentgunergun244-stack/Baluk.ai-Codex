...
function buildIncrementalHtmlArtifact(prompt = "", advanced = false, existingCode = "") {
  ...
  const plan = buildHtmlActionPlan(prompt);
  const lower = String(prompt || "").toLocaleLowerCase("tr-TR");
  const actionNotes = [];
...
function buildEngineArtifact(prompt = "", language = "html", model = "baluk-2.2", existingCode = "") {
  ...
  if (language === "python") return buildPythonEngineArtifact(prompt, advanced, existingCode);
...
async function submitEnginePrompt() {
  ...
  appendEngineEntry("assistant", `Komutunu aldim: "${prompt}"`, "Baluk Engine");
  enginePrompt.value = "";
  const statuses = getEngineStatuses(prompt, active.language, active.model);
  ...
  active.updatedAt = Date.now();
  appendEngineEntry("assistant", `${artifact.summary} T'den ciktiyi, N'den kodu gorebilirsin.`, `${active.model === "baluk-2.2-thinking" ? "Thinking" : "Normal"} • ${active.language === "html" ? "HTML/CSS/JS" : "PYTHON"}`);
  setEngineStatus(
