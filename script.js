...
if (engineIntro) {
  engineIntro.addEventListener("click", (e) => {
...
function renderEngineLog() {
  ...
  engineLog.innerHTML = "";
  engineIntro.classList.add("hidden");
...
function renderEngineWorkspace() {
  ...
  if (engineStatusCard) engineStatusCard.classList.toggle("hidden", !engineRunning && !active.entries.length && !enginePanelMode);
...
async function submitEnginePrompt() {
  ...
  const artifact = buildEngineArtifact(prompt, active.language, active.model);
